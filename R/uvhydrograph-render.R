...
createPrimaryPlot <- function(data, month){ 
  ...
  if(anyDataExist(primaryData)){
    ...
    plot_object <- gsplot(ylog=primaryInfo$logAxis, yaxs='r') %>% 
      lines(as.POSIXct(NA), as.POSIXct(NA), xlim=c(primaryInfo$plotDates[1], 
...
createSecondaryPlot <- function(data, month){
  ...
  sec_plot_object <- gsplot(yaxs='r') %>% 
    lines(as.POSIXct(NA), as.POSIXct(NA), xlim=c(secondaryInfo$plotDates[1], 
  ...
  for (i in 1:length(secondaryData)) {
    ...
    for (j in seq_len(length(secondaryStyles))) {
      sec_plot_object <- do.call(names(secondaryStyles[j]), append(list(object=sec_plot_object), secondaryStyles[[j]]))
    }
    
  }
  
  ncol <- ifelse(length(sec_plot_object$legend) > 3, 2, 1)
  leg_lines <- ifelse(ncol==2, ceiling((length(sec_plot_object$legend) - 6)/2), 0) 
  legend_offset <- ifelse(ncol==2, 0.3+(0.05*leg_lines), 0.3)
  
  sec_plot_object <- legend(sec_plot_object, location="below", title="", ncol=ncol, 
                            legend_offset=legend_offset, cex=0.8) %>% 
  ...
  if(isShift){
    sec_plot_object <- sec_plot_object %>% 
      mtext(paste0(secondaryInfo$tertiary_lbl, " (", secondaryInfo$sec_units, ")"), 
                          side = 4, line = 1.5) %>% 
      axis(side=4, las=0)
  }
  
  sec_plot_object <- testCallouts(sec_plot_object, xlimits = xlim(sec_plot_object)$side.1)

  table <- correctionsTable(secondaryData)
  
  ##HACKY FIX FOR OVERLAPPING LABELS###
  sec_plot_object$view.1.2$window$xlim <- as.numeric(sec_plot_object$view.1.2$window$xlim)
  sec_plot_object$view.1.2$window$ylim <- as.numeric(sec_plot_object$view.1.2$window$ylim)
  if(isShift){sec_plot_object$view.1.4$window$xlim <- sec_plot_object$view.1.2$window$xlim}
  
  return(list(plot=sec_plot_object, table=table))
}

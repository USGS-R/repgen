getFiveyearStyle <- function(data, info=NULL) {
    ...
    styles <- switch(names(data), 
                     ...
                     max_iv = list(points = list(x=x, y=y, col="red", pch=8, cex=2, legend.name=paste("Min. Instantaneous", info$type, ":", y))),
                     min_iv = list(points = list(x=x, y=y, col="blue", pch=8, cex=2, legend.name=paste("Max. Instantaneous", info$type, ":", y))),

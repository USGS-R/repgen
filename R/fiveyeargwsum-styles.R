getFiveyearStyle <- function(data) {
    ...
    styles <- switch(names(data), 
                     ...
                     max_iv = list(points = list(x=x, y=y, col="red", pch=8, cex=2, legend.name=paste("Min. (Lowest) Instantaneous Water Level:", y))),
                     min_iv = list(points = list(x=x, y=y, col="blue", pch=8, cex=2, legend.name=paste("Max. (Highest) Instantaneous Water Level:", y))),

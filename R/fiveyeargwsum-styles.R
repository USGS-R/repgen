getFiveyearStyle <- function(data) {
    ...
    styles <- switch(names(data), 
                     ...
                     max_iv = list(points = list(x=x, y=y, col="red", pch=8, cex=2, legend.name=paste("Min. Instantaneous (Lowest) Water Level:", y)),
                                   callouts = list(x=x, y=y, labels = y)),
                     min_iv = list(points = list(x=x, y=y, col="blue", pch=8, cex=2, legend.name=paste("Max. Instantaneous (Highest) Water Level:", y)),
                                   callouts = list(x=x, y=y, labels = y)),

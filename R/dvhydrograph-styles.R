...
getDvStyle <- function(data, x, y){
  ...
  styles <- switch(names(data), 
                   ...
                   final_dv = list(lines = list(x=x, y=y, col="black", type='l', lty=1, lwd=1, legend.name="Final Daily Values")), 
                   #ref_dv = list(lines = list(x=x, y=y, col="blue", type='l', lty=1, lwd=1, legend.name="")),

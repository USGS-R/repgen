getDvStyle <- function(data, info = NULL, ...){
  ...
  styles <- switch(names(data), 
                   ...
                   est_stat1 = list(lines = list(x=x, y=y, type="s", col="red", lty=2, pch=20, cex=0.5, legend.name=legend.name)),
                   est_stat2 = list(lines = list(x=x, y=y, type="s", col="red", lty=3, pch=20, cex=0.5, legend.name=legend.name)),
                   est_stat3 = list(lines = list(x=x, y=y, type="s", col="red", lty=6, pch=20, cex=0.5, legend.name=legend.name)),

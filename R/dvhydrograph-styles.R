getDvStyle <- function(data){
  ...
  styles <- switch(names(data), 
                   ...
                   secondary_ref = list(lines = list(x=x, y=y, col="blue", type='l', lty=1, lwd=1, legend.name=data[[1]]$legend.name)),
                   tertiary_ref = list(lines = list(x=x, y=y, col="orange", type='l', lty=1, lwd=1, legend.name=data[[1]]$legend.name)),
                   quaternary_ref = list(lines = list(x=x, y=y, col="purple", type='l', lty=1, lwd=1, legend.name=data[[1]]$legend.name)),

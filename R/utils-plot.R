...
#' Plot a time series on a fourth axis
#' @title printWithFourthYAxis
#' @param plot A gsplot, plot object.
#' @seealso \code{\link{log_tick_marks}}
printWithFourthYAxis <- function(plot) {
  axisDistance <- 5
  ...
  plot(NA, 0, ylim=ylim(plot, side=6), xlim=c(0,1), axes=FALSE, ylab="", xlab="")
  axis(side=4, at=ticks, line=axisDistance, las=0, append=TRUE)
  mtext(plot$side.6$label, side=4, line=axisDistance+1, padj=0)
  
  #do it all again for 4th y axis
  mar_vals <- par(plot)$mar
  if(is.null(mar_vals)){
    mar_vals <- par('mar')
  } 
  ...
  print(plot)
  plot
  # minor.ticks <- pretty(ylim(plot,7))
  # major.ticks <- log_tick_marks(ylim(plot,7)[[1]], ylim(plot,7)[[2]])
  # major.ticks <- major.ticks[major.ticks <= ylim(plot, 7)[[2]]]
  # ticks <- major.ticks[major.ticks >= ylim(plot, 7)[[1]]]
  # 
  # #Only add additional tick marks if we don't have many generated by the log ticks function
  # if(length(ticks) <= 3)
  # {
  ...
  # #Only remove decimals from tick marks if the y range is great enough that they're not necessary
  # #Add points below and above the plot to extaned the axis bar to the full plot height
  # if((ylim(plot,7)[[2]]-ylim(plot,7)[[1]]) >= 10)
  # {
  #   ticks <- trunc(append(ticks, c(-1, ylim(plot,7)[[2]]*2)))
  # } else {
  #   ticks <- append(ticks, c(-1, ylim(plot,7)[[2]]*2))
  # }
  
  #Draw the 4th axis
  par(new=TRUE)
  plot(NA, 0, ylim=ylim(plot, side=7), xlim=c(0,1), axes=FALSE, ylab="", xlab="")
  axis(side=4, at=ticks, line=axisDistance, las=0, append=TRUE)
  mtext(plot$side.7$label, side=4, line=axisDistance+1.5, padj=0)
}

...
getUvStyle <- function(data, info, x, y, approvalInfo, correctionLabels, plotName) {
  ...
  if (plotName == "secondary"){
    ...
    styles <- switch(names(data),
                ...
                gage_height = list(points=list(x=x, y=y, pch=21, bg='black', col='black', cex=.8, lwd=1, legend.name="Gage height measurement", axes=FALSE),
                                   text=list(x=x, y=y, labels=data$gage_height$n, pos=4)),
                gw_level = list(x=x,y=y, pch = 8, bg = 'orange', col = 'orange', cex = 1.2, lwd=1, legend.name="Measured Water Level (NWIS-RA)", axes=FALSE), 
                meas_shift = list(points=list(x=x, y=y, pch=21, bg='green', col='green', cex=1, lwd=1, legend.name="Effective shift and error", axes=FALSE),
